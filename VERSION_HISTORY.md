...
## AI Native 2.0 - 2026-09-25

### 核心转变
从"分类路由"到"能力筛选"，从"向量猜文件"到"全量目录 + LLM 自选"。

### 架构对比
| 维度 | `AI Native 1.0` | `AI Native 2.0` |
|------|--------------|--------------|
| 路由 | 5 选 1 分类 | 0~N 能力候选 |
| 数据源 | 向量 Top1 → 注入 | 全量目录 → 自选 |
| 决策位置 | Router 前置 | LLM 在线 |
| 新工具 | 改 prompt | 自动发现 |
| 多步骤 | 受限单一分类 | 多能力并行 |
...
| `ui/v3.3.0` | 2026-09-15 | 输入性能优化：消息列表 useMemo 缓存，长会话输入不再卡顿 |
| `ui/v3.4.0` | 2026-09-15 | 消息管理增强：复制 / 导出对话 / 单条导出 / 清空对话 |
| `ui/v3.4.1` | 2026-09-17 | 修复环境变量未隔离导致的 Render 线上图表 404 问题 |
| `ui/v3.5.0` | 2026-09-22 | 观察者权限拦截（日志/知识库）+ 用户身份展示（部门+角色映射）+ 健康仪表板进度条 |
