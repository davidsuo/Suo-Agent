...
with gr.Blocks(title="AI 智能体") as demo:
        ...
        with gr.Row(elem_id="top-nav-container"):
            with gr.Tabs(elem_id="top-nav-bar") as main_tabs:
                        ...
                        with gr.Column(scale=3, elem_id="chat-main-area"):
                            ...
                            with gr.Group(elem_id="input-card"):
                                with gr.Row(elem_id="file-row"):
                                    ...
                                    attachment_html = gr.Markdown(value="", visible=True, elem_id="attachment-html")
                                    clear_file_btn = gr.Button("×", scale=0, min_width=40, elem_id="clear-btn", visible=False)
                                with gr.Row(elem_id="input-row-final"):
                                    text_input = gr.Textbox(show_label=False, placeholder="发消息或按住空格说话，松开发送...", scale=1)
                                    input_spacer = gr.Markdown("", scale=3, elem_id="input-row-spacer")
                                    file_upload_btn = gr.UploadButton("📎", file_types=[".csv", ".xlsx", ".xls", ".png", ".jpg", ".jpeg", ".bmp", ".gif", ".wav", ".mp3", ".m4a", ".ogg"], scale=0, min_width=60, elem_id="upload-icon-btn")
                ...
                with gr.Tab("用户管理", visible=False) as user_management_tab:
                    ...
                    users_table = gr.Dataframe(headers=["用户名", "姓名", "部门", "职位", "角色", "租户"], interactive=False)
                ...
                with gr.Tab("知识库"):
                    ...
                    with gr.Row():
                        kb_upload = gr.File(label="上传知识文档", file_types=[".txt", ".md", ".csv", ".pdf", ".docx"], type="filepath")
        ...
        if session_username:
            ...
            if user:
                ...
                return (user, gr.update(visible=False), gr.update(visible=True), hist if hist else [],
                        ...
                        session_project or "主对话", new_project_names, "",
                        f"**当前用户：{user_full}**", gr.update(visible=(user.get("role") == "admin")),
                        f"**当前用户：** {user_full}", gr.update(visible=(user.get("role") == "admin")))
        return (None, gr.update(visible=True), gr.update(visible=False), [],
                gr.Dropdown(choices=get_available_tenants(), value="default"),
                gr.update(choices=["主对话"], value="主对话"), "主对话", ["主对话"],
                "", "", gr.update(visible=False), "**当前用户：** 未登录", gr.update(visible=False))

    demo.load(fn=load_history, inputs=[session_user_input],
    ...
    file_upload_btn.upload(fn=handle_file_upload, inputs=[file_upload_btn], outputs=[pending_file, attachment_html, clear_file_btn], show_progress="hidden")

    def clear_file():
        return None, "", gr.update(visible=False)

    clear_file_btn.click(fn=clear_file, inputs=[], outputs=[pending_file, attachment_html, clear_file_btn], show_progress="hidden")

    add_project_btn.click(fn=lambda: (gr.update(visible=False), gr.update(visible=True)), inputs=None, outputs=[add_project_btn, project_creation_row], show_progress="hidden")
    cancel_project_btn.click(fn=lambda: (gr.update(visible=True), gr.update(visible=False)), inputs=None, outputs=[add_project_btn, project_creation_row], show_progress="hidden")
    ...
    def delete_project(current_project, project_names):
        ...
        if new_names:
            ...
            return (gr.update(choices=new_names, value=new_current), new_current, new_names)
        else:
            return (gr.update(choices=["主对话"], value="主对话"), "主对话", ["主对话"])

    delete_project_btn.click(fn=delete_project, inputs=[current_project, project_names], outputs=[project_list, current_project, project_names], show_progress="hidden")
    ...
    voice_file_input.upload(fn=voice_upload_handler, inputs=[text_input, chatbot, voice_file_input, user_state, current_project],
    ...
    def refresh_status():
        ...
        for w in workers:
            stats = w.get_stats()
            data.append([stats["name"], str(stats["is_running"]), stats["task_count"], stats["error_count"], stats["queue_size"], stats["avg_time"], stats["error_rate"]])
...
if __name__ == "__main__":
    ...
    demo.launch(
        ...
        css="""
            ...
            #project-sidebar, #project-sidebar .block, #project-sidebar .wrap, #project-sidebar .gr-box, #project-sidebar .form {
            ...
            #project-list, #project-list .block, #project-list .wrap, #project-list .gr-box, #project-list .form {
                background: transparent !important;
                border: none !important;
                box-shadow: none !important;
                padding: 0 !important;
            ...
            #attachment-html, #attachment-html .block, #attachment-html .wrap, #attachment-html .gr-box, #attachment-html .form {
                background: transparent !important;
                border: none !important;
                box-shadow: none !important;
                margin: 0 !important;
