...
class RAGV2Evaluator:
    ...
    async def evaluate_sample(self, sample: Dict) -> Dict:
        ...
        if q_type == "negative":
            passed = final_metrics.get("rejection_accuracy", 0) >= METRICS_THRESHOLDS["rejection_accuracy"]
        else:
            passed = (
                final_metrics.get("context_recall", 0) >= METRICS_THRESHOLDS["context_recall"] and
                final_metrics.get("context_precision", 0) >= METRICS_THRESHOLDS["context_precision"]
            )
        return {"id": sample["id"], "query": query, "type": q_type, "ground_truth": gt_ids, 
                "retrieved_ids": retrieved_ids, "generated_answer": generated_answer, 
    ...
    def _compute_id_based_metrics(self, retrieved_ids, gt_ids, q_type):
        ...
        gt_set = set(gt_ids)
        if q_type == "negative":
            rejection_accuracy = 1.0 if len(retrieved_set) == 0 else 0.0
            return {"context_recall": 0.0, "context_precision": 0.0, "rejection_accuracy": rejection_accuracy}
        if len(gt_set) == 0:
        ...
        return {"context_recall": round(context_recall, 4), "context_precision": round(context_precision, 4), "rejection_accuracy": 0.0}

    async def _compute_llm_relevancy(self, query, answer, retrieved_ids):
        if not answer or not retrieved_ids: return 0.0
    ...
    def generate_report(self):
        if not self.results:
            return {"total_samples": 0, "passed_samples": 0, "pass_rate": 0.0, "average_metrics": {k: 0.0 for k in METRICS_THRESHOLDS}, "results": []}
        avg_metrics = {}
        ...
        return {"evaluation_time": datetime.utcnow().isoformat(), "total_samples": total_count, "passed_samples": passed_count,
                "pass_rate": round(passed_count / total_count, 4) if total_count else 0, "average_metrics": avg_metrics,
...
class RealRAGClient:
    ...
    async def generate(self, query: str):
        if not HTTPX_AVAILABLE: return {"answer": "httpx未安装", "contexts": []}
...
class MockRAGClient:
    async def generate(self, query):
        if "打印机报错" in query or "0x80004005" in query: return {"answer": "...", "contexts": ["IT-01"]}
        elif "共享打印机" in query: return {"answer": "...", "contexts": ["IT-05"]}
        elif "下午茶" in query or "报销" in query: return {"answer": "未找到相关文档", "contexts": []}
        else: return {"answer": "通用处理办法", "contexts": ["IT-99"]}
...
async def main():
    ...
    if args.real: print("⚠️ 警告：正在连接真实后端服务...")
